        ...
        acquireAccessToken()
            .then((response) => {
                ...
                if (userHasRequiredRole)
                    fetch('/api/issues?select=id,title,service,details,startDateTime,lastModifiedDateTime,isResolved,origin,classification,status&filter=isResolved eq false',
                        ...
                        .then(result => {
                            if (result && result.length > 0) {
                                var serviceNames = Array.from(new Set(result.map((a: any) => a.service)));

                                var services = serviceNames.map((svc: any) => {
                                    var svcIssueCollection = result.filter((i: any) => i.service === svc);
                                    return {
                                        service: svc,
                                        activeIssues: svcIssueCollection ? svcIssueCollection.length : 0
                                    };
                                });
                                var sortedResult = services.sort((a: any, b: any) => new Date(a.activeIssues) > new Date(b.activeIssues) ? -1 : 1);
