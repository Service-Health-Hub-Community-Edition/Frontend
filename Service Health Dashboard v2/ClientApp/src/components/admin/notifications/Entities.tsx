...
import { IComponent } from './Routing';
...
export class Entities extends React.Component<{}, IEntityState> {
        ...
        const columns: ICompositeListColumn[] = [
            {
                ...
                onRender: (item: IComponent) => {
                    return <Link href={'/admin/notifications/routing?entityId=' + item.id} styles={pipeFabricStyles}>{item.name}</Link >;
                }
            }
        ];
                ...
                <div className="container" style={{ maxWidth: '97%' }}>
                    <div className="row">
                        <div className="col">
                            ...
                            <DetailPageHeader
                                title="Components"
                                description="Select a component from the list below to configure notification settings."
                            />
                        </div>
                    </div>
                    <div className="row">
                    ...
                    fetch('/api/Components', { headers: authResponse.idToken === "" ? {} : { 'Authorization': `Bearer ${authResponse.idToken}` } })
                        ...
                        .then(result => {
                            for (const component of result) {
                                if (component.capabilities?.includes('Routing') && component.capabilities?.includes('Notifications'))
