...
export class TaskMappingEntities extends React.Component<{}, IEntityState> {
        ...
        const columns: ICompositeListColumn[] = [
            {
                ...
                onRender: (item: IComponent) => {
                    return <Link href={'/admin/mapping/editor?entityId=' + item.id} styles={pipeFabricStyles}>{item.name}</Link >;
                }
            }
        ];
                        ...
                        <div className="col">
                            <M365Breadcrumb
                                items={[
                                    ...
                                    { text: 'Components', key: 'components', href: '/admin/mapping', isCurrentItem: true }
                                ]}
                                style={{ marginBottom: '16px' }}
                            />
                            <DetailPageHeader
                                title="Components"
                                description="Select a component from the list below to configure task field mapping settings."
                            />
                        </div>
                    </div>
                    <div className="row">
                    ...
                    fetch('/api/Components', { headers: authResponse.idToken === "" ? {} : { 'Authorization': `Bearer ${authResponse.idToken}` } })
                        ...
                        .then(result => {
                            for (const component of result) {
                                if (component.capabilities?.includes('TaskManagement'))
